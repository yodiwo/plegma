...
public class NodeService extends IntentService {
    ...
    private static final String EXTRA_HTTP_DATA = "EXTRA_HTTP_DATA";
    private static final String EXTRA_HTTP_METHOD = "EXTRA_HTTP_METHOD";
    public static final String EXTRA_HTTP_REQUEST_RESPONSE = "EXTRA_HTTP_REQUEST_RESPONSE";
    ...
    private static final int REQUEST_HTTP_SEND = 32;
    ...
    public static final String BROADCAST_HTTP_REQUEST_SEND = "NodeService.BROADCAST_HTTP_REQUEST_SEND";
    public static final String BROADCAST_NODE_SERVICE_TEARDOWN = "NodeService.BROADCAST_NODE_SERVICE_TEARDOWN";
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (settingsProvider.getTransportProtocol().equals("1")) {
            isMqttSelected = true;
            //Helpers.log(Log.DEBUG, TAG, "Mqtt selected");
        }
        else if (settingsProvider.getTransportProtocol().equals("2")){
            isGcmSelected = true;
            //Helpers.log(Log.DEBUG, TAG, "Gcm selected");
        }
        else{
            isMqttSelected = true;
            //Helpers.log(Log.DEBUG, TAG, "Mqtt selected by default");
        }
        ...
        if (request_type == REQUEST_STARTUP) {
            ...
            LocalBroadcastManager
                    .getInstance(context)
                    .sendBroadcast(new Intent(BROADCAST_NODE_SERVICE_TEARDOWN));
            this.stopSelf();
            return;
            ...
            return;
        }else if (request_type == REQUEST_HTTP_SEND){
            try {
                String uri = bundle.getString(EXTRA_URI);
                String data = bundle.getString(EXTRA_HTTP_DATA);
                String method = bundle.getString(EXTRA_HTTP_METHOD);

                String response = SendHttpRequest(method, data, uri);
                if (response!= null && !response.isEmpty()) {
                    Intent intentHTTP = new Intent(BROADCAST_HTTP_REQUEST_SEND);
                    intentHTTP.putExtra(EXTRA_HTTP_REQUEST_RESPONSE, response);
                    LocalBroadcastManager
                            .getInstance(context)
                            .sendBroadcast(intentHTTP);
                }
            } catch (Exception ex) {
        ...
        try {
            ...
            if (isMqttSelected) {
                if (mqttServerAPI.SendReq(msg))
                    Helpers.log(Log.DEBUG, TAG, "Mqtt msg sent: revNum = " + localThingsRevNum + ", #Things = " + thingHashMap.size());
            }
            else if (isGcmSelected) {
                if (gcmServerAPI.SendReq(msg))
                    Helpers.log(Log.DEBUG, TAG, "GCM msg sent: revNum = " + localThingsRevNum + ", #Things = " + thingHashMap.size());
            }
        } catch (Exception e) {
            Helpers.logException(TAG, e);
        }
    ...
    private void UpdateThing(Thing thing) {
        ...
        try {
            ...
            if (isMqttSelected) {
                if (mqttServerAPI.SendReq(msg))
                    Helpers.log(Log.DEBUG, TAG, "Mqtt msg sent: revNum = " + localThingsRevNum);
            }
            else if (isGcmSelected) {
                if (gcmServerAPI.SendReq(msg))
                    Helpers.log(Log.DEBUG, TAG, "GCM msg sent: revNum = " + localThingsRevNum);
            }
        } catch (Exception e) {
            Helpers.logException(TAG, e);
        }
    ...
    private void SendPortMsg(Thing thing, String portIndex, String data, int qos) {
        if (data != null) {
            ...
            try {
                ...
                if (rc)
                    Helpers.log(Log.INFO, TAG, "Sent portEventMsg for thing " + thing.Name + " with state " + data);
                else
                    Helpers.log(Log.ERROR, TAG, "Could not send portEventMsg for thing " + thing.Name + " with state " + data);
    ...
    private void SendPortMsg(Thing thing, String[] data, int qos) {
        ...
        if (data != null && data.length > 0) {
            ...
            try {
                ...
                if (rc)
                    Helpers.log(Log.INFO, TAG, "Sent portEvent array for thing " + thing.Name);
                else
                    Helpers.log(Log.ERROR, TAG, "Could not send portEvent array for thing " + thing.Name);
            } catch (Exception e) {
                Helpers.logException(TAG, e);
            }
        }
    ...
    private void SendMyHackingReq(MyHackingReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent MyHackingReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send Sent MyHackingReq");
    ...
    private void SendShareActionReq(ShareActionReq[] req, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent ShareThingsRequest");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send ShareThingsRequest");
    ...
    private void SendGetUserTokensReq(GetUserTokensReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent GetUserTokensReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send GetUserTokensReq");
    ...
    private void SendAddFriendReq(AddFriendReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent AddFriendReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send AddFriendReq");
    ...
    private void SendGetUserInfoReq(GetUserInfoReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent GetUserInfoReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send GetUserInfoReq");
    ...
    private void SendGetFriendsReq(GetFriendsReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent GetFriendsReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send GetFriendsReq");
    ...
    private void SendGetThingsReq(GetThingsReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent GetThingsReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send GetThingsReq");
    ...
    private void SendGraphActionReq(GraphActionReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent GraphActionReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send GraphActionReq");
    ...
    private void SendGetNodeInfoReq(GetNodeInfoReq msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent GetNodeInfoReq");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send GetNodeInfoReq");
    ...
    private void SendLiveKeysAddMsg(LiveKeysAddMsg msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent LiveKeysAddMsg");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send LiveKeysAddMsg");
    ...
    private void SendLiveKeysDelMsg(LiveKeysDelMsg msg, int qos) {
        ...
        try {
            ...
            if (rc)
                Helpers.log(Log.INFO, TAG, "Sent LiveKeysDelMsg");
            else
                Helpers.log(Log.ERROR, TAG, "Could not send LiveKeysDelMsg");
                        ...
                        public void Handle(Object msg, int syncId, int flags) {
                            ...
                            if (thingsset.Operation == ThingsSet.Update) {
                                ...
                                for (Thing thing: thingsset.Data){
                                    RegisterThingAction(getApplicationContext(), new Thing[]{thing}, RegisterOperation.ADD);
    ...
    public static void SendHttpRequest(Context context, String method, String uri, String data){
        Helpers.log(Log.INFO, TAG, method + " HTTP request, uri: " + uri + data);

        Intent intent = new Intent(context, NodeService.class);
        intent.putExtra(EXTRA_REQUEST_TYPE, REQUEST_HTTP_SEND);
        intent.putExtra(EXTRA_URI, uri);
        intent.putExtra(EXTRA_HTTP_DATA, data);
        intent.putExtra(EXTRA_HTTP_METHOD, method);
        context.startService(intent);
    }
    ...
    private String SendHttpRequest(String method, String data, String uri){
        Helpers.log(Log.DEBUG, TAG, "Send Http request");

        HttpURLConnection connection = null;
        StringBuilder response = new StringBuilder();
        try {
            if (method.equalsIgnoreCase("GET")) {
                URL url = new URL(uri + data);
                connection = (HttpURLConnection) url.openConnection();
                connection.setReadTimeout(10000);
                connection.setConnectTimeout(15000);
                connection.setRequestMethod("GET");
                ...
                int responseCode = connection.getResponseCode();
                Helpers.log(Log.DEBUG, TAG, "Sending 'GET' request to URL : " + url);
                Helpers.log(Log.DEBUG, TAG, "Response Code : " + responseCode);
                if (responseCode != HttpsURLConnection.HTTP_OK) {
                    return null;
                }
            }
            ...
            Helpers.log(Log.DEBUG, TAG, "Server response : " + response.toString());
        } catch (Exception ex) {
            Helpers.logException(TAG, ex);
        } finally {
            if (connection != null)
    ...
    private void displayNotification(Context context, Class activity, int NotificationId, String title, String msg[]) {
        ...
        try {
            ...
            if (NotificationId == ACTIVE_PORT_UPDATE_ID) {
                ...
                Helpers.log(Log.WARN, TAG, "Not valid notification ID");
            }
        }catch (Exception ex){
            Helpers.logException(TAG, ex);
        }
    ...
    public static boolean isActive(String portkey){
        return ActivePortKeysHashSet.contains(portkey);
    }

    public static boolean isActive(ArrayList<Port> ports){
        for (Port port : ports){
            if (ActivePortKeysHashSet.contains(port.PortKey))
                return true;
        }
        return false;
    }
