...
public class GcmRxService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        switch (request_type) {
            ...
            // -----------------------------------
            default:
                Log.e(TAG, "GcmRxService received unknown action: " + request_type);
        }
    }
     ...
     */
     private void sendRegistrationToServer(String token) throws IOException {
         Log.d(TAG, "Sending Registration Id / GCM connection notification to the Yodiwo server");
         ...
         {
             Log.d(TAG, "sending of GcmConnectionMsg failed");
         }
     }

    private void sendDisconnectionToServer(String token) throws IOException {
        Log.d(TAG, "Sending GCM disconnection notification to the Yodiwo server");
        String nodeKey = settingsProvider.getNodeKey();
        String secretKey = settingsProvider.getNodeSecretKey();
        ...
        if (rsp == null || rsp.getStatus() != 200)
        {
            Log.d(TAG, "sending of GcmDisconnectionMsg failed");
        }
    }
