	...
	size_t *up_data_size, void **ptr)
{
	...
	if (strcmp(method, MHD_HTTP_METHOD_GET) == 0) {
		if (strcmp(url, "/") == 0 ||
			...
			ret = web_send_file(conn, "/home/pi/open-zwave1/ozwcpUI/cp.html", MHD_HTTP_OK, false);
		else if (strcmp(url, "/scenes.html") == 0)
			ret = web_send_file(conn, "scenes.html", MHD_HTTP_OK, false);
		else if (strcmp(url, "/cp.js") == 0)
			ret = web_send_file(conn, "/home/pi/open-zwave1/ozwcpUI/cp.js", MHD_HTTP_OK, false);
		else if (strcmp(url, "/favicon.png") == 0)
			ret = web_send_file(conn, "/home/pi/open-zwave1/ozwcpUI/openzwavetinyicon.png", MHD_HTTP_OK, false);
...
Webserver::Webserver(int const wport, char* deviceName) : sortcol(COL_NODE), logbytes(0), adminstate(false)
{
	fprintf(stderr, "webserver starting port %d\n", wport);
	port = wport;
	wdata = MHD_start_daemon(MHD_USE_THREAD_PER_CONNECTION | MHD_USE_DEBUG, port,
	...
	devname = NULL;
	devname = (char *)malloc(strlen(deviceName) + 1);
	strcpy(devname, deviceName);
	cout << "Device name in constructor" << devname << endl;
}
