...
function wsapi(cfg) {
    ...
    */
    ...
    this.portState = function (op, ports) {
        ...
        if (Object.prototype.toString.call(ports) === '[object Array]') {
            ports.forEach(function (port) {

                payload.PortKeys.push(self.nodeKey + '-' + port);
            });
            console.log(1);

        } else {
            payload.PortKeys.push(self.nodeKey + '-' + ports);
        }
    ...
    var handleThingGet = function (data, syncId) {
        //TODO:HANDLE ERROR CASES ThingKey, treq.Operation
        if (self.onThingGet && typeof (self.onThingGet) === "function") {
    ...
    var handleNodeUnpairedMsg = function (reasonCode, message) {
                    ...
                    case ws_msgtype.API:
                        switch (request.SubId) {
                            ...
                            case "nodeinforeq":
                                ...
                                break;
                            case "nodeunpairedmsg":
                                handleNodeUnpairedMsg(payload.ReasonCode, payload.Message);
                                break;
                            case "thingsreq":
                                switch (payload.Operation) {
                                    case ws_thingoperation.GET:
                                        handleThingGet(payload, request.SyncId);
                                        break;
                                    case ws_thingoperation.UPDATE:
                                        handleThingUpdate(payload, request.SyncId);
                                        break;
                                    case ws_thingoperation.OVERWRITE:
                                        handleThingCreate(payload, request.SyncId);
                                        break;
                                    case ws_thingoperation.DELETE:
                                        handleThingDelete(payload, request.SyncId);
                                        break;
                                }
                                break;
                            case "thingsrsp":
                                if (payload.status == false)
                                    alert("ThingsReq failed");
                                break;
                            case "porteventmsg":
