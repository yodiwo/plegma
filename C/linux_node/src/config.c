...
int read_config(APIGenerator_AdditionalTypes_CNodeCConfig_t *config, char *filename)
{
    ...
    r = APIGenerator_AdditionalTypes_CNodeCConfig_FromJson(buf, size, config);
exit:
    free(buf);
    if (f != NULL)
    	fclose(f);
...
int write_config(APIGenerator_AdditionalTypes_CNodeCConfig_t *config, char *filename)
{
    ...
    r = APIGenerator_AdditionalTypes_CNodeCConfig_ToJson(buf, 2048, config);
    if (r < 0) 
        goto exit;
    fwrite(buf, r - 1, 1, f);
exit:
...
//			|| config->ActiveID == NULL
		...
		return NULL;
	return &config->Configs.elems[config->ActiveID];

//	int i;
//	for (i = 0; i < config->Configs.num; i++)
//	{
//		if (!strcmp(config->ActiveID, config->Configs.elems[i].Name)) {
//			return &config->Configs.elems[i];
//		}
//	}
//	return NULL;
}
