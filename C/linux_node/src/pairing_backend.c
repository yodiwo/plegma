...
int pairing_context_init_with_defaults(pairing_context *ctx,
        onPaired_callback callback)
{
...
extern APIGenerator_AdditionalTypes_CNodeConfig_t *activeConfig;

int pairing_context_init_from_config(pairing_context *ctx,
        onPaired_callback callback)
{
...
{
    int len = strlen(hostname) + strlen(urlBase) + 20; //TODO: proper calculation
    char* result = (char *) malloc(sizeof(char) * len);
    if (!result)
        return NULL;
    sprintf(result, "http://%s:%d%snext", hostname, (port == 0) ? 80 : port,
            urlBase);
    return result;
}
...
{
    int len = strlen(ctx->postUrl) + strlen(ctx->token2) + strlen(ctx->next_url) + sizeof(PAGE_SERVER_PHASE2) + 40; //TODO: proper calculation
    char* result = (char *) malloc(sizeof(char) * len);
    if (!result)
        return NULL;
    sprintf(result, "%s" "1/" PAGE_SERVER_PHASE2 "?token2=%s&noderedirect=%s",
            ctx->postUrl, ctx->token2, ctx->next_url);
    return result;
}
